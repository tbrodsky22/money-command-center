...
function friendlyHeader(){const id=activeId(),m=meta[id];if(m&&window.pageTitle){pageTitle.textContent=m[0];pageSub.textContent=m[1]}const sel=document.querySelector('.mobileSectionSelect');if(sel)sel.value=id}
...
function init(){
 ...
 const observer=new MutationObserver(friendlyHeader);if(window.pageTitle)observer.observe(pageTitle,{childList:true,subtree:true});friendlyHeader();
}
